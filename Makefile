...
.PHONY: accept check story prove rust elixir app stats portfolio inventory cross-domain errors points invchain credit full audit contribution quota badge invflow pfflow cb pq tpq vr sf apc da rsf wc dvr di awc et eit awccb etv eif fbs etr eifr fbs2 etrc eifrc fbe etrcc ready deploy all
...
rsf: ## 四链联动对账（Python /receive-ship-fillrate-test + Rust 86/86 + Elixir 四链联动 3/3，v0.361）
...
dvr: ## 双资产估值-风险四链对账（Python /dual-asset-vr-test + Rust 91/91 + Elixir 双资产估值-风险四链 4/4，v0.381）
	$(PYTHON) impl/python/sigma_app.py --dual-asset-vr-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-dvr

di: ## 双货品四链对账（Python /dual-item-four-link-test + Rust 93/93 + Elixir 双货品四链 4/4，v0.391）
	$(PYTHON) impl/python/sigma_app.py --dual-item-four-link-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-di

awc: ## 验收-提现-契分-勋章四链对账（Python /accept-withdraw-credit-badge-test + Rust 102/102 + Elixir 四链联动 4/4，v0.409）
	$(PYTHON) impl/python/sigma_app.py --accept-withdraw-credit-badge-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-awc

et: ## 双资产等量买卖对消链对账（Python /dual-asset-equal-trade-test + Rust 104/104 + Elixir 双资产等量买卖对消链 4/4，v0.419）
	$(PYTHON) impl/python/sigma_app.py --dual-asset-equal-trade-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-et

eit: ## 双货品等量入出对消链对账（Python /dual-item-equal-trade-test + Rust 106/106 + Elixir 双货品等量入出对消链 3/3，v0.429）
	$(PYTHON) impl/python/sigma_app.py --dual-item-equal-trade-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-eit

awccb: ## 验收-提现-契分-贡献-勋章五链守恒对账（Python /accept-withdraw-credit-contribution-badge-test + Rust 111/111 + Elixir 五链守恒 5/5，v0.439）
	$(PYTHON) impl/python/sigma_app.py --accept-withdraw-credit-contribution-badge-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-awccb

etv: ## 双资产等量买卖对消-估值-风险五链对账（Python /dual-asset-equal-trade-vr-test + Rust 114/114 + Elixir 五链 4/4，v0.449）
	$(PYTHON) impl/python/sigma_app.py --dual-asset-equal-trade-vr-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-etv

eif: ## 双货品等量入出对消-水位-履约五链对账（Python /dual-item-equal-trade-fillrate-test + Rust 117/117 + Elixir 五链 4/4，v0.459）
	$(PYTHON) impl/python/sigma_app.py --dual-item-equal-trade-fillrate-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-eif

fbs: ## 全业务链六链守恒对账（Python /full-business-six-link-test + Rust 123/123 + Elixir 六链守恒 6/6，v0.469）
	$(PYTHON) impl/python/sigma_app.py --full-business-six-link-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-fbs

etr: ## 双资产等量买卖对消-估值-风险-恢复六链对账（Python /dual-asset-equal-trade-vr-restore-test + Rust 127/127 + Elixir 六链 5/5，v0.479）
...
fbs2: ## 全业务链七链守恒对账（Python /full-business-seven-link-test + Rust 138/138 + Elixir 七链守恒 7/7，v0.499）
	$(PYTHON) impl/python/sigma_app.py --full-business-seven-link-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-fbs2

etrc: ## 双资产等量买卖对消-估值-风险-恢复-对消循环七链对账（Python /dual-asset-equal-trade-vr-restore-cycle-test + Rust 143/143 + Elixir 七链 6/6，v0.509）
	$(PYTHON) impl/python/sigma_app.py --dual-asset-equal-trade-vr-restore-cycle-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-etrc

eifrc: ## 双货品等量入出对消-水位-履约-恢复-对消循环七链对账（Python /dual-item-equal-trade-fillrate-restore-cycle-test + Rust 148/148 + Elixir 七链 6/6，v0.519）
	$(PYTHON) impl/python/sigma_app.py --dual-item-equal-trade-fillrate-restore-cycle-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-eifrc

fbe: ## 全业务链八链守恒对账（Python /full-business-eight-link-test + Rust 156/156 + Elixir 八链守恒 8/8，v0.529）
	$(PYTHON) impl/python/sigma_app.py --full-business-eight-link-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-fbe

etrcc: ## 双资产等量买卖对消-估值-风险-恢复-对消循环-现金守恒八链对账（Python /dual-asset-equal-trade-vr-restore-cycle-cash-test + Rust 162/162 + Elixir 八链 7/7，v0.539）
	$(PYTHON) impl/python/sigma_app.py --dual-asset-equal-trade-vr-restore-cycle-cash-test
	cd impl/verifier && cargo run -q -- --app-smoke
	cd impl/elixir_rt && elixir sigma_verify.exs --sk-etrcc
