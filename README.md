...
三端共识 51/51、数学证明 80 项 PROVED、找茬产品一条命令就能启动上线
...
> 安装：`pip install sigma-lang`（发布到 PyPI 后）——装完即可
...
python3 tools/sigma-prove.py corpus/socketkit_ok.md corpus/portfolio_ok.md corpus/inventory_ok.md  # z3 义务消解（80 项 PROVED）
...
```text
  ...
  corpus/ 语料（51 个模块：ok 期望 PASS，break 期望 FAIL）
   ...
   │
   ├──▶ tools/sigma-prove.py     z3 义务消解（80 项 PROVED）
...
| `tools/sigma-prove.py` | 把语料定律编码为 z3 义务并消解 | 80 项 PROVED |
...
| 6 | 健康检查（v0.74） | `GET /health` | status ok + 门禁摘要（51/51、73 PROVED） |
...
Verifier Consensus / 验证器共识: **51/51** corpus modules agree across Python / Rust / Elixir verifiers.
51/51 语料库模块在 Python / Rust / Elixir 三个验证器上达成一致。

**v0.89 收官总览 (2026-08-04)**: 协议 **spec 0.5.0**，三域（§SK 找茬业务 /
§PF 金融 / §IN 供应链）——consensus **51/51**、p0 **109/109**、sigma-prove
**80 项 PROVED**（29 模块）、sigma-runtime **59/59 + 47/47**（--domains 含
...
§PF 金融 / §IN 供应链）——consensus **51/51**、p0 **109/109**、sigma-prove
**80 项 PROVED**（29 模块）、sigma-runtime **59/59 + 55/55**（--domains 含
