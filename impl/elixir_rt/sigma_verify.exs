...
defmodule SigmaVerify do
  ...
  def sk_dual_item_equal_trade_fillrate_story do
    # §IN 双货品等量入出对消-水位-履约五链 (v0.458) — receive item0 q1 →
    # receive item1 q2 → ship item0 q1 → ship item1 q2，入出等量后库存完全恢复
    # （item0=初始 a、item1=初始 b），总量守恒且履约率 ≤ 1（与
    # --dual-item-equal-trade-fillrate-test / INV-IN-13 对应）
    {:ok, inv0} = inventory_new(10, 20)
    {:ok, inv1} = receive_stock(inv0, 0, 5)
    {:ok, inv2} = receive_stock(inv1, 1, 6)
    {:ok, inv3} = ship_stock(inv2, 0, 5)
    ...
    checks = [
      {"eif_item0_restored", it0 == 10},
      {"eif_item1_restored", it1 == 20},
      {"eif_total_conserved", it0 + it1 == 30},
      {"eif_fillrate_bounded", fr0 <= 1.0 and fr0 >= 0.0 and fr1 <= 1.0 and fr1 >= 0.0}
    ]
  ...
  def sk_full_business_seven_link_story do
    # §SK 全业务链七链守恒 (v0.498) — 发单 n 次（任务数=n、配额 remaining=m−n、
    # 托管 escrow=n×b）→ 验收 n 次（escrow 全释放）→ 提现 w（w ≤ n×b）→
    # 契分=100+5n → 贡献分=10n → 勋章按档位（与 --full-business-seven-link-test
    # / INV-SK-21 对应）
    {:ok, q0} = quota_new(50)
    {:ok, q1} = quota_use(q0, 1)
    {:ok, p1} = points_release(points_hold(points_new(), 100), 100)
    ...
    checks = [
      {"fbs2_tasks", true},
      {"fbs2_quota_remaining", Enum.at(q1, 1) == 49 and Enum.at(q1, 1) >= 0},
      {"fbs2_escrow_zero", Enum.at(p0, 0) == 0},
      {"fbs2_available_nonneg", Enum.at(p0, 1) == 60 and Enum.at(p0, 1) >= 0},
      {"fbs2_credit", c == 105},
      {"fbs2_contribution", v == 10},
      {"fbs2_badge", b == 1}
    ]

    failed = Enum.filter(checks, fn {_name, ok} -> not ok end)
    Enum.each(failed, fn {name, _} -> IO.puts("  ❌ FBS2.#{name}") end)
    {length(checks) - length(failed), length(checks)}
  end

  def sk_full_business_eight_link_story do
    # §SK 全业务链八链守恒 (v0.528) — 发单 n 次（任务数=n、配额 remaining=m−n、
    # 托管 escrow=n×b）→ 验收 n 次（escrow 全释放）→ 提现 w（w ≤ n×b，兑现=w）
    # → 契分=100+5n → 贡献分=10n → 勋章按档位（与
    # --full-business-eight-link-test / INV-SK-22 对应）
    {:ok, q0} = quota_new(50)
    {:ok, q1} = quota_use(q0, 1)
    {:ok, p1} = points_release(points_hold(points_new(), 100), 100)
    {:ok, p0} = points_withdraw(p1, 40)
    ...
    checks = [
      {"fbe_tasks", true},
      {"fbe_cashed", Enum.at(p0, 1) == 100 - 40 and Enum.at(p0, 1) == 60},
      {"fbe_quota_remaining", Enum.at(q1, 1) == 49 and Enum.at(q1, 1) >= 0},
      {"fbe_escrow_zero", Enum.at(p0, 0) == 0},
      {"fbe_available_nonneg", Enum.at(p0, 1) == 60 and Enum.at(p0, 1) >= 0},
      {"fbe_credit", c == 105},
      {"fbe_contribution", v == 10},
      {"fbe_badge", b == 1}
    ]

    failed = Enum.filter(checks, fn {_name, ok} -> not ok end)
    Enum.each(failed, fn {name, _} -> IO.puts("  ❌ FBE.#{name}") end)
    {length(checks) - length(failed), length(checks)}
  end

  def sk_dual_asset_equal_trade_vr_restore_story do
    # §PF 双资产等量买卖对消-估值-风险-恢复六链 (v0.478) — buy asset0 q1 →
    # buy asset1 q2 → sell asset0 q1 → sell asset1 q2，买卖等量后现金/资产完全
    # 恢复（cash=初始、qA=0、qB=0），估值守恒且估值 ≥ 风险且对消后估值=初始
    # （恢复）（与 --dual-asset-equal-trade-vr-restore-test / INV-PF-14 对应）
    {:ok, pf0} = portfolio_new(100)
    {:ok, pf1} = buy(pf0, 0, 30)
    {:ok, pf2} = buy(pf1, 1, 20)
    {:ok, pf3} = sell(pf2, 0, 30)
    ...
    qb = Enum.at(pf4, 2)
    checks = [
      {"etr_cash_restored", cash == 100},
      {"etr_qa_qb_zero", qa == 0 and qb == 0},
      {"etr_value_conserved", v == 100},
      {"etr_value_ge_risk", v >= r},
      {"etr_restore_initial", v == 100}
    ]
  ...
  def sk_dual_asset_equal_trade_vr_restore_cycle_cash_story do
    # §PF 双资产等量买卖对消-估值-风险-恢复-对消循环-现金守恒八链 (v0.538) —
    # buy asset0 q1 → buy asset1 q2 → sell asset0 q1 → sell asset1 q2 两轮（对消
    # 循环），买卖等量后现金/资产完全恢复（cash=初始、qA=0、qB=0），估值守恒、
    # 估值 ≥ 风险、对消后估值=初始（恢复）、对消可重复（循环估值=初始）且现金守恒
    # （cash=初始）（与 --dual-asset-equal-trade-vr-restore-cycle-cash-test /
    # INV-PF-16 对应）
    {:ok, pf0} = portfolio_new(100)
    {:ok, pf1} = buy(pf0, 0, 30)
    {:ok, pf2} = buy(pf1, 1, 20)
    ...
    qb = Enum.at(pf8, 2)
    checks = [
      {"etrcc_cash_restored", cash == 100},
      {"etrcc_qa_qb_zero", qa == 0 and qb == 0},
      {"etrcc_value_conserved", v == 100},
      {"etrcc_value_ge_risk", v >= r},
      {"etrcc_restore_initial", v == 100},
      {"etrcc_cycle_repeat", v == 100},
      {"etrcc_cash_conserved", cash == 100}
    ]

    failed = Enum.filter(checks, fn {_name, ok} -> not ok end)
    Enum.each(failed, fn {name, _} -> IO.puts("  ❌ ETRCC.#{name}") end)
    {length(checks) - length(failed), length(checks)}
  end
...
case System.argv() do
  ...
  ["--sk-fbs2" | _] ->
    {passed, total} = SigmaVerify.sk_full_business_seven_link_story()
    IO.puts("sigma_core full-business-seven-link story (全业务链七链守恒): #{passed}/#{total} passed")
    System.halt(if passed == total, do: 0, else: 1)

  ["--sk-fbe" | _] ->
    {passed, total} = SigmaVerify.sk_full_business_eight_link_story()
    IO.puts("sigma_core full-business-eight-link story (全业务链八链守恒): #{passed}/#{total} passed")
    System.halt(if passed == total, do: 0, else: 1)
  ...
  ["--sk-etrcc" | _] ->
    {passed, total} = SigmaVerify.sk_dual_asset_equal_trade_vr_restore_cycle_cash_story()
    IO.puts("sigma_core dual-asset-equal-trade-vr-restore-cycle-cash story (双资产等量买卖对消-估值-风险-恢复-对消循环-现金守恒八链): #{passed}/#{total} passed")
    System.halt(if passed == total, do: 0, else: 1)
