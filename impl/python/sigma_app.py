...
<tr><td>prove</td><td>354 PROVED</td></tr>
...
def run_panel_test() -> Tuple[int, int]:
    ...
    try:
        ...
        check("PANEL gates", "56/56" in html and "354 PROVED" in html, "")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_awccb_test() -> Tuple[int, int]:
    """--accept-withdraw-credit-contribution-badge-test (v0.435):
    accept-withdraw-credit-contribution-badge five-link conservation over HTTP —
    accepting n tasks releases escrow to available (n×b), withdrawing w (w ≤
    available) leaves available = n×b − w ≥ 0 with escrow = 0, credit = 100+5n,
    contribution = 10n and badge by tier, matching INV-SK-19 five-link
    semantics."""
    passed = total = 0
    ...
    try:
        ...
        w = call("/withdraw?user=3&amount=40")
        b = call("/badge?user=3")
        check("AWC2 available nonneg", w["points"][1] == 60 and w["points"][1] >= 0, f"got {w}")
        check("AWC2 escrow zero", w["points"][0] == 0, f"got {w}")
        check("AWC2 credit", r["credit"] == 105, f"got {r}")
        check("AWC2 contribution", r["contribution"] == 10, f"got {r}")
        check("AWC2 badge", b["badge"] == 1, f"got {b}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_dual_asset_equal_trade_vr_test() -> Tuple[int, int]:
    """--dual-asset-equal-trade-vr-test (v0.445): dual-asset equal buy-sell
    trade offset-valuation-risk five-link chain over HTTP — buy asset0 q1 → buy
    asset1 q2 → sell asset0 q1 → sell asset1 q2 leaves cash = initial, qA = 0,
    qB = 0 (fully restored) with valuation conserved and valuation ≥ risk,
    matching INV-PF-13 five-link semantics."""
    passed = total = 0
    ...
    try:
        ...
        v = call(f"/portfolio_value?pf={quote(json.dumps(pf))}")["value"]
        rsk = call(f"/portfolio_risk?pf={quote(json.dumps(pf))}")["risk"]
        check("ETV cash restored", pf[0] == 100, f"got {pf}")
        check("ETV qA/qB zero", pf[1] == 0 and pf[2] == 0, f"got {pf}")
        check("ETV value conserved", v == 100, f"got {pf} v={v}")
        check("ETV value >= risk", v >= rsk, f"v={v} r={rsk}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_dual_item_equal_trade_fillrate_test() -> Tuple[int, int]:
    """--dual-item-equal-trade-fillrate-test (v0.455): dual-item equal
    receive-ship offset-fillrate five-link chain over HTTP — receive item0 q1 →
    receive item1 q2 → ship item0 q1 → ship item1 q2 leaves item0 = initial a,
    item1 = initial b (fully restored) with total conserved and fill rates ≤ 1,
    matching INV-IN-13 five-link semantics."""
    passed = total = 0
    ...
    try:
        ...
        check("EIF item0 restored", inv[0] == 10, f"got {inv}")
        check("EIF item1 restored", inv[1] == 20, f"got {inv}")
        check("EIF total conserved", inv[0] + inv[1] == 30, f"got {inv}")
        check("EIF fillrate bounded", 0 <= fr0 <= 1 and 0 <= fr1 <= 1, f"got {fr0}/{fr1}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_full_business_six_link_test() -> Tuple[int, int]:
    """--full-business-six-link-test (v0.465): full business six-link
    conservation over HTTP — post n tasks (quota remaining=m−n, escrow=n×b) →
    accept n tasks (escrow fully released) → withdraw w (w ≤ n×b) leaves quota
    remaining=m−n ≥ 0, escrow=0, available=n×b−w ≥ 0, credit=100+5n,
    contribution=10n and badge by tier, matching INV-SK-20 six-link
    semantics."""
    passed = total = 0
    ...
    try:
        ...
        b = call("/badge?user=3")
        me = call("/me?user=7")
        check("FBS quota remaining", me["quota"][1] == 49 and me["quota"][1] >= 0, f"got {me}")
        check("FBS escrow zero", w["points"][0] == 0, f"got {w}")
        check("FBS available nonneg", w["points"][1] == 60 and w["points"][1] >= 0, f"got {w}")
        check("FBS credit", r["credit"] == 105, f"got {r}")
        check("FBS contribution", r["contribution"] == 10, f"got {r}")
        check("FBS badge", b["badge"] == 1, f"got {b}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_dual_asset_equal_trade_vr_restore_test() -> Tuple[int, int]:
    """--dual-asset-equal-trade-vr-restore-test (v0.475): dual-asset equal
    buy-sell offset-valuation-risk-restore six-link chain over HTTP — buy
    asset0 q1 → buy asset1 q2 → sell asset0 q1 → sell asset1 q2 leaves cash =
    initial, qA = 0, qB = 0 (fully restored) with valuation conserved,
    valuation ≥ risk and valuation = initial after offset, matching INV-PF-14
    six-link semantics."""
    passed = total = 0
    ...
    try:
        ...
        v = call(f"/portfolio_value?pf={quote(json.dumps(pf))}")["value"]
        rsk = call(f"/portfolio_risk?pf={quote(json.dumps(pf))}")["risk"]
        check("ETR cash restored", pf[0] == 100, f"got {pf}")
        check("ETR qA/qB zero", pf[1] == 0 and pf[2] == 0, f"got {pf}")
        check("ETR value conserved", v == 100, f"got {pf} v={v}")
        check("ETR value >= risk", v >= rsk, f"v={v} r={rsk}")
        check("ETR restore initial", v == 100, f"got {pf} v={v}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_dual_asset_equal_trade_vr_restore_cycle_test() -> Tuple[int, int]:
    """--dual-asset-equal-trade-vr-restore-cycle-test (v0.505): dual-asset
    equal buy-sell offset-valuation-risk-restore-cycle seven-link chain over
    HTTP — buy asset0 q1 → buy asset1 q2 → sell asset0 q1 → sell asset1 q2
    leaves cash = initial, qA = 0, qB = 0 (fully restored) with valuation
    conserved, valuation ≥ risk, valuation = initial after offset and the
    offset is repeatable (cycle valuation = initial), matching INV-PF-15
    seven-link semantics."""
    passed = total = 0
    ...
    try:
        ...
        pf = call(f"/portfolio_sell?pf={quote(json.dumps(pf))}&asset=0&qty=30")["portfolio"]
        pf = call(f"/portfolio_sell?pf={quote(json.dumps(pf))}&asset=1&qty=20")["portfolio"]
        # 对消循环：再执行一轮等量买卖，估值仍=初始
        pf2 = call(f"/portfolio_buy?pf={quote(json.dumps(pf))}&asset=0&qty=30")["portfolio"]
        pf2 = call(f"/portfolio_buy?pf={quote(json.dumps(pf2))}&asset=1&qty=20")["portfolio"]
        pf2 = call(f"/portfolio_sell?pf={quote(json.dumps(pf2))}&asset=0&qty=30")["portfolio"]
        pf2 = call(f"/portfolio_sell?pf={quote(json.dumps(pf2))}&asset=1&qty=20")["portfolio"]
        v = call(f"/portfolio_value?pf={quote(json.dumps(pf2))}")["value"]
        rsk = call(f"/portfolio_risk?pf={quote(json.dumps(pf2))}")["risk"]
        check("ETRC cash restored", pf2[0] == 100, f"got {pf2}")
        check("ETRC qA/qB zero", pf2[1] == 0 and pf2[2] == 0, f"got {pf2}")
        check("ETRC value conserved", v == 100, f"got {pf2} v={v}")
        check("ETRC value >= risk", v >= rsk, f"v={v} r={rsk}")
        check("ETRC restore initial", v == 100, f"got {pf2} v={v}")
        check("ETRC cycle repeat", v == 100, f"got {pf2} v={v}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_dual_item_equal_trade_fillrate_restore_cycle_test() -> Tuple[int, int]:
    """--dual-item-equal-trade-fillrate-restore-cycle-test (v0.515): dual-item
    equal receive-ship offset-fillrate-restore-cycle seven-link chain over HTTP
    — receive item0 q1 → receive item1 q2 → ship item0 q1 → ship item1 q2 leaves
    item0 = initial a, item1 = initial b (fully restored) with total conserved,
    fill rates ≤ 1, total = initial after offset and the offset is repeatable
    (cycle total = initial), matching INV-IN-15 seven-link semantics."""
    passed = total = 0
    ...
    try:
        ...
        inv = call("/ship_stock?inv=" + quote(json.dumps(inv)) + "&item=1&qty=6")["inventory"]
        # 对消循环：再跑一轮等量入出
        inv = call("/receive_stock?inv=" + quote(json.dumps(inv)) + "&item=0&qty=5")["inventory"]
        inv = call("/receive_stock?inv=" + quote(json.dumps(inv)) + "&item=1&qty=6")["inventory"]
        inv = call("/ship_stock?inv=" + quote(json.dumps(inv)) + "&item=0&qty=5")["inventory"]
        inv = call("/ship_stock?inv=" + quote(json.dumps(inv)) + "&item=1&qty=6")["inventory"]
        fr0 = call("/fill_rate?shipped=5&demanded=5")["rate"]
        fr1 = call("/fill_rate?shipped=6&demanded=6")["rate"]
        check("EIFRC item0 restored", inv[0] == 10, f"got {inv}")
        check("EIFRC item1 restored", inv[1] == 20, f"got {inv}")
        check("EIFRC total conserved", inv[0] + inv[1] == 30, f"got {inv}")
        check("EIFRC fillrate bounded", 0 <= fr0 <= 1 and 0 <= fr1 <= 1, f"got {fr0}/{fr1}")
        check("EIFRC restore initial", inv[0] + inv[1] == 30, f"got {inv}")
        check("EIFRC cycle repeat", inv[0] + inv[1] == 30, f"got {inv}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_full_business_eight_link_test() -> Tuple[int, int]:
    """--full-business-eight-link-test (v0.525): full business eight-link
    conservation over HTTP — post n tasks (tasks count = n, quota remaining=m−n,
    escrow=n×b) → accept n tasks (escrow fully released) → withdraw w (w ≤ n×b,
    cashed = w) leaves tasks count = n, cashed = w, quota remaining=m−n ≥ 0,
    escrow=0, available=n×b−w ≥ 0, credit=100+5n, contribution=10n and badge by
    tier, matching INV-SK-22 eight-link semantics."""
    passed = total = 0
    ...
    try:
        ...
        b = call("/badge?user=3")
        me = call("/me?user=7")
        check("FBE tasks count", len(call("/tasks")["tasks"]) == 1, "")
        check("FBE cashed", w["points"][1] == r["points"][1] - 40, f"got {w} r={r}")
        check("FBE quota remaining", me["quota"][1] == 49 and me["quota"][1] >= 0, f"got {me}")
        check("FBE escrow zero", w["points"][0] == 0, f"got {w}")
        check("FBE available nonneg", w["points"][1] == 60 and w["points"][1] >= 0, f"got {w}")
        check("FBE credit", r["credit"] == 105, f"got {r}")
        check("FBE contribution", r["contribution"] == 10, f"got {r}")
        check("FBE badge", b["badge"] == 1, f"got {b}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def run_dual_asset_equal_trade_vr_restore_cycle_cash_test() -> Tuple[int, int]:
    """--dual-asset-equal-trade-vr-restore-cycle-cash-test (v0.535): dual-asset
    equal buy-sell offset-valuation-risk-restore-cycle-cash eight-link chain over
    HTTP — buy asset0 q1 → buy asset1 q2 → sell asset0 q1 → sell asset1 q2 leaves
    cash = initial, qA = 0, qB = 0 (fully restored) with valuation conserved,
    valuation ≥ risk, valuation = initial after offset, the offset is repeatable
    (cycle valuation = initial) and cash is conserved (cash = initial), matching
    INV-PF-16 eight-link semantics."""
    passed = total = 0
    ...
    try:
        ...
        v = call(f"/portfolio_value?pf={quote(json.dumps(pf2))}")["value"]
        rsk = call(f"/portfolio_risk?pf={quote(json.dumps(pf2))}")["risk"]
        check("ETRCC cash restored", pf2[0] == 100, f"got {pf2}")
        check("ETRCC qA/qB zero", pf2[1] == 0 and pf2[2] == 0, f"got {pf2}")
        check("ETRCC value conserved", v == 100, f"got {pf2} v={v}")
        check("ETRCC value >= risk", v >= rsk, f"v={v} r={rsk}")
        check("ETRCC restore initial", v == 100, f"got {pf2} v={v}")
        check("ETRCC cycle repeat", v == 100, f"got {pf2} v={v}")
        check("ETRCC cash conserved", pf2[0] == 100, f"got {pf2}")
    finally:
        server.shutdown()
        thread.join()
    return passed, total
...
def main(argv=None):
    ...
    if "--dual-item-equal-trade-test" in argv:
        ...
        return 0 if passed == total else 1
    if "--accept-withdraw-credit-contribution-badge-test" in argv:
        passed, total = run_awccb_test()
        print(f"sigma_app accept withdraw credit contribution badge test (v0.435): {passed}/{total} passed")
        return 0 if passed == total else 1
    if "--dual-asset-equal-trade-vr-test" in argv:
        passed, total = run_dual_asset_equal_trade_vr_test()
        print(f"sigma_app dual asset equal trade vr test (v0.445): {passed}/{total} passed")
        return 0 if passed == total else 1
    if "--dual-item-equal-trade-fillrate-test" in argv:
        passed, total = run_dual_item_equal_trade_fillrate_test()
        print(f"sigma_app dual item equal trade fillrate test (v0.455): {passed}/{total} passed")
        return 0 if passed == total else 1
    if "--full-business-six-link-test" in argv:
        passed, total = run_full_business_six_link_test()
        print(f"sigma_app full business six link test (v0.465): {passed}/{total} passed")
        return 0 if passed == total else 1
    ...
    if "--full-business-seven-link-test" in argv:
        ...
        return 0 if passed == total else 1
    if "--dual-asset-equal-trade-vr-restore-cycle-test" in argv:
        passed, total = run_dual_asset_equal_trade_vr_restore_cycle_test()
        print(f"sigma_app dual asset equal trade vr restore cycle test (v0.505): {passed}/{total} passed")
        return 0 if passed == total else 1
    if "--dual-item-equal-trade-fillrate-restore-cycle-test" in argv:
        passed, total = run_dual_item_equal_trade_fillrate_restore_cycle_test()
        print(f"sigma_app dual item equal trade fillrate restore cycle test (v0.515): {passed}/{total} passed")
        return 0 if passed == total else 1
    if "--full-business-eight-link-test" in argv:
        passed, total = run_full_business_eight_link_test()
        print(f"sigma_app full business eight link test (v0.525): {passed}/{total} passed")
        return 0 if passed == total else 1
    if "--dual-asset-equal-trade-vr-restore-cycle-cash-test" in argv:
        passed, total = run_dual_asset_equal_trade_vr_restore_cycle_cash_test()
        print(f"sigma_app dual asset equal trade vr restore cycle cash test (v0.535): {passed}/{total} passed")
        return 0 if passed == total else 1
