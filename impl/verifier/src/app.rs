...
pub fn run_smoke() -> (usize, usize) {
    ...
    // 33. 双资产混合交易链估值守恒对账 (v0.349) — buy(0,30)→buy(1,20)→
    //     sell(0,10)→sell(1,5) 后估值 cash+qA+qB 守恒且 cash/qA/qB ≥ 0
    //     （与 Python --dual-asset-test 对应，INV-PF-10 语义）
    let mut pfd = http_get(port, "/portfolio_new?cash=100")["portfolio"].clone();
    pfd = http_get(port, &format!("/portfolio_buy?pf={}&asset=0&qty=30",
        serde_json::to_string(&pfd).unwrap_or_default()))["portfolio"].clone();
    pfd = http_get(port, &format!("/portfolio_buy?pf={}&asset=1&qty=20",
        serde_json::to_string(&pfd).unwrap_or_default()))["portfolio"].clone();
    pfd = http_get(port, &format!("/portfolio_sell?pf={}&asset=0&qty=10",
        serde_json::to_string(&pfd).unwrap_or_default()))["portfolio"].clone();
    pfd = http_get(port, &format!("/portfolio_sell?pf={}&asset=1&qty=5",
        serde_json::to_string(&pfd).unwrap_or_default()))["portfolio"].clone();
    let vd = http_get(port, &format!("/portfolio_value?pf={}",
        serde_json::to_string(&pfd).unwrap_or_default()))["value"].as_i64().unwrap_or(0);
    let cd = pfd[0].as_i64().unwrap_or(-1);
    let qa = pfd[1].as_i64().unwrap_or(-1);
    ...
    // 35. 提现-契分联动对账 (v0.369) — 验收后提现 w：available 增加
    //     n×b−w、escrow 净不变（post +100 后 accept −100）、契分联动
    //     （与 Python --withdraw-credit-test 对应，INV-SK-16 语义）
    let escrow_before4 = http_get(port, "/stats")["platform_points"][0].as_i64().unwrap_or(0);
    let avail_before4 = http_get(port, "/stats")["platform_points"][1].as_i64().unwrap_or(0);
    let _ = http_get(port, "/post?author=7&bounty=100");
    let tid16 = http_get(port, "/tasks")["tasks"].as_array()
        .and_then(|a| a.last())
        .map(|t| t["task_id"].as_u64().unwrap_or(0)).unwrap_or(0);
    let _ = http_get(port, &format!("/claim?task={tid16}&hunter=3"));
    let _ = http_get(port, &format!("/submit?task={tid16}"));
    let _ = http_get(port, &format!("/accept?task={tid16}&caller=7"));
    let w16 = http_get(port, "/withdraw?user=3&amount=40");
    let p16 = w16["points"].clone();
    let c16 = http_get(port, "/badge?user=3")["credit"].as_i64().unwrap_or(0);
    check!("HTTP /wc_chain available",
           p16[1].as_i64().unwrap_or(-1) == avail_before4 + 60
    ...
    // 38. 全业务链五链守恒对账 (v0.399) — 发单→验收→提现后：配额 remaining
    //     ≥ 0、escrow=0、available ≥ 0、契分/贡献分联动（与 Python
    //     --full-business-five-link-test 对应，INV-SK-17 语义）
    let escrow_before5 = http_get(port, "/stats")["platform_points"][0].as_i64().unwrap_or(0);
    let avail_before5 = http_get(port, "/stats")["platform_points"][1].as_i64().unwrap_or(0);
    let _ = http_get(port, "/post?author=7&bounty=100");
    let tid17 = http_get(port, "/tasks")["tasks"].as_array()
        .and_then(|a| a.last())
        .map(|t| t["task_id"].as_u64().unwrap_or(0)).unwrap_or(0);
    let _ = http_get(port, &format!("/claim?task={tid17}&hunter=3"));
    let _ = http_get(port, &format!("/submit?task={tid17}"));
    let r17 = http_get(port, &format!("/accept?task={tid17}&caller=7"));
    let w17 = http_get(port, "/withdraw?user=3&amount=40");
    let p17 = w17["points"].clone();
    let q17 = http_get(port, "/me?user=7")["quota"][1].as_i64().unwrap_or(-1);
    check!("HTTP /fbl_chain quota", q17 >= 0 && q17 <= 50);
    check!("HTTP /fbl_chain escrow",
           p17[0].as_i64().unwrap_or(-1) == escrow_before5);
    check!("HTTP /fbl_chain available",
           p17[1].as_i64().unwrap_or(-1) == avail_before5 + 60
           && p17[1].as_i64().unwrap_or(-1) >= 0);
    check!("HTTP /fbl_chain credit",
           r17["credit"].as_i64().unwrap_or(0) >= 100 + 5);
    check!("HTTP /fbl_chain contribution", r17["contribution"].as_i64().unwrap_or(0) >= 10);

    // 39. 验收-提现-契分-勋章四链联动对账 (v0.407) — 验收后提现 w：available
    //     增加 n×b−w ≥0、escrow 净不变、契分/勋章联动（与 Python
    //     --accept-withdraw-credit-badge-test 对应，INV-SK-18 语义）
    let escrow_before6 = http_get(port, "/stats")["platform_points"][0].as_i64().unwrap_or(0);
    let avail_before6 = http_get(port, "/stats")["platform_points"][1].as_i64().unwrap_or(0);
    let _ = http_get(port, "/post?author=7&bounty=100");
    let tid18 = http_get(port, "/tasks")["tasks"].as_array()
        .and_then(|a| a.last())
        .map(|t| t["task_id"].as_u64().unwrap_or(0)).unwrap_or(0);
    let _ = http_get(port, &format!("/claim?task={tid18}&hunter=3"));
    let _ = http_get(port, &format!("/submit?task={tid18}"));
    let r18 = http_get(port, &format!("/accept?task={tid18}&caller=7"));
    let w18 = http_get(port, "/withdraw?user=3&amount=40");
    let p18 = w18["points"].clone();
    let b18 = http_get(port, "/badge?user=3")["badge"].as_i64().unwrap_or(0);
    check!("HTTP /awc_chain available",
           p18[1].as_i64().unwrap_or(-1) == avail_before6 + 60
           && p18[1].as_i64().unwrap_or(-1) >= 0);
    check!("HTTP /awc_chain escrow", p18[0].as_i64().unwrap_or(-1) == escrow_before6);
    ...
    // 41. 双货品等量入出对消链对账 (v0.427) — receive(0,5)→receive(1,6)→
    //     ship(0,5)→ship(1,6) 后库存完全恢复、总量守恒（与 Python
    //     --dual-item-equal-trade-test 对应，INV-IN-12 语义）
    let _ = http_get(port, "/inventory_new?qty_a=10&qty_b=20");
    let inv12 = http_get(port, "/receive_stock?inv=[10,20]&item=0&qty=5")["inventory"].clone();
    let inv12b = http_get(port, &format!("/receive_stock?inv={}&item=1&qty=6",
        serde_json::to_string(&inv12).unwrap_or_default()))["inventory"].clone();
    let inv12c = http_get(port, &format!("/ship_stock?inv={}&item=0&qty=5",
        serde_json::to_string(&inv12b).unwrap_or_default()))["inventory"].clone();
    let inv12d = http_get(port, &format!("/ship_stock?inv={}&item=1&qty=6",
        serde_json::to_string(&inv12c).unwrap_or_default()))["inventory"].clone();
    let it0_12 = inv12d[0].as_i64().unwrap_or(-1);
    ...
    // 43. 双资产等量买卖对消-估值-风险五链对账 (v0.447) — buy(0,30)→buy(1,20)
    //     →sell(0,30)→sell(1,20) 后现金/资产完全恢复、估值守恒且估值 ≥ 风险
    //     （与 Python --dual-asset-equal-trade-vr-test 对应，INV-PF-13 语义）
    let mut pfe2 = http_get(port, "/portfolio_new?cash=100")["portfolio"].clone();
    pfe2 = http_get(port, &format!("/portfolio_buy?pf={}&asset=0&qty=30",
        serde_json::to_string(&pfe2).unwrap_or_default()))["portfolio"].clone();
    pfe2 = http_get(port, &format!("/portfolio_buy?pf={}&asset=1&qty=20",
        serde_json::to_string(&pfe2).unwrap_or_default()))["portfolio"].clone();
    pfe2 = http_get(port, &format!("/portfolio_sell?pf={}&asset=0&qty=30",
        serde_json::to_string(&pfe2).unwrap_or_default()))["portfolio"].clone();
    pfe2 = http_get(port, &format!("/portfolio_sell?pf={}&asset=1&qty=20",
        serde_json::to_string(&pfe2).unwrap_or_default()))["portfolio"].clone();
    let ve2 = http_get(port, &format!("/portfolio_value?pf={}",
        serde_json::to_string(&pfe2).unwrap_or_default()))["value"].as_i64().unwrap_or(0);
    let re2 = http_get(port, &format!("/portfolio_risk?pf={}",
        serde_json::to_string(&pfe2).unwrap_or_default()))["risk"].as_i64().unwrap_or(0);
    let ce2 = pfe2[0].as_i64().unwrap_or(-1);
    let qa_e2 = pfe2[1].as_i64().unwrap_or(-1);
    let qb_e2 = pfe2[2].as_i64().unwrap_or(-1);
    check!("HTTP /etv_chain restored", ce2 == 100 && qa_e2 == 0 && qb_e2 == 0);
    check!("HTTP /etv_chain value", ve2 == 100);
    check!("HTTP /etv_chain risk", ve2 >= re2);

    // 44. 双货品等量入出对消-水位-履约五链对账 (v0.457) — receive(0,5)→
    //     receive(1,6)→ship(0,5)→ship(1,6) 后库存完全恢复、总量守恒且履约率
    //     ≤ 1（与 Python --dual-item-equal-trade-fillrate-test 对应，INV-IN-13
    //     语义）
    let _ = http_get(port, "/inventory_new?qty_a=10&qty_b=20");
    let inv13 = http_get(port, "/receive_stock?inv=[10,20]&item=0&qty=5")["inventory"].clone();
    let inv13b = http_get(port, &format!("/receive_stock?inv={}&item=1&qty=6",
        serde_json::to_string(&inv13).unwrap_or_default()))["inventory"].clone();
    let inv13c = http_get(port, &format!("/ship_stock?inv={}&item=0&qty=5",
        serde_json::to_string(&inv13b).unwrap_or_default()))["inventory"].clone();
    let inv13d = http_get(port, &format!("/ship_stock?inv={}&item=1&qty=6",
        serde_json::to_string(&inv13c).unwrap_or_default()))["inventory"].clone();
    let fr0_13 = http_get(port, "/fill_rate?shipped=5&demanded=5")["rate"].as_f64().unwrap_or(-1.0);
    let fr1_13 = http_get(port, "/fill_rate?shipped=6&demanded=6")["rate"].as_f64().unwrap_or(-1.0);
    let it0_13 = inv13d[0].as_i64().unwrap_or(-1);
    let it1_13 = inv13d[1].as_i64().unwrap_or(-1);
    check!("HTTP /eif_chain restored", it0_13 == 10 && it1_13 == 20);
    check!("HTTP /eif_chain total", it0_13 + it1_13 == 30);
    check!("HTTP /eif_chain fillrate", (0.0..=1.0).contains(&fr0_13) && (0.0..=1.0).contains(&fr1_13));

    // 45. 全业务链六链守恒对账 (v0.467) — 发单→验收→提现后：配额 remaining
    //     ≥ 0、escrow=0、available ≥ 0、契分/贡献/勋章联动（与 Python
    //     --full-business-six-link-test 对应，INV-SK-20 语义）
    let escrow_before8 = http_get(port, "/stats")["platform_points"][0].as_i64().unwrap_or(0);
    let avail_before8 = http_get(port, "/stats")["platform_points"][1].as_i64().unwrap_or(0);
    let _ = http_get(port, "/post?author=7&bounty=100");
    let tid20 = http_get(port, "/tasks")["tasks"].as_array()
        .and_then(|a| a.last())
        .map(|t| t["task_id"].as_u64().unwrap_or(0)).unwrap_or(0);
    let _ = http_get(port, &format!("/claim?task={tid20}&hunter=3"));
    let _ = http_get(port, &format!("/submit?task={tid20}"));
    let r20 = http_get(port, &format!("/accept?task={tid20}&caller=7"));
    let w20 = http_get(port, "/withdraw?user=3&amount=40");
    let p20 = w20["points"].clone();
    let q20 = http_get(port, "/me?user=7")["quota"][1].as_i64().unwrap_or(-1);
    let b20 = http_get(port, "/badge?user=3")["badge"].as_i64().unwrap_or(0);
    check!("HTTP /fbs_chain quota", q20 >= 0 && q20 <= 50);
    check!("HTTP /fbs_chain escrow",
           p20[0].as_i64().unwrap_or(-1) == escrow_before8);
    check!("HTTP /fbs_chain available",
           p20[1].as_i64().unwrap_or(-1) == avail_before8 + 60
           && p20[1].as_i64().unwrap_or(-1) >= 0);
    check!("HTTP /fbs_chain credit",
           r20["credit"].as_i64().unwrap_or(0) >= 100 + 5);
    check!("HTTP /fbs_chain contribution", r20["contribution"].as_i64().unwrap_or(0) >= 10);
    check!("HTTP /fbs_chain badge", b20 >= 1);

    // 46. 双资产等量买卖对消-估值-风险-恢复六链对账 (v0.477) — buy(0,30)→
    //     buy(1,20)→sell(0,30)→sell(1,20) 后现金/资产完全恢复、估值守恒且
    //     估值 ≥ 风险、对消后估值=初始（与 Python
    //     --dual-asset-equal-trade-vr-restore-test 对应，INV-PF-14 语义）
    let mut pfe3 = http_get(port, "/portfolio_new?cash=100")["portfolio"].clone();
    pfe3 = http_get(port, &format!("/portfolio_buy?pf={}&asset=0&qty=30",
        serde_json::to_string(&pfe3).unwrap_or_default()))["portfolio"].clone();
    pfe3 = http_get(port, &format!("/portfolio_buy?pf={}&asset=1&qty=20",
        serde_json::to_string(&pfe3).unwrap_or_default()))["portfolio"].clone();
    pfe3 = http_get(port, &format!("/portfolio_sell?pf={}&asset=0&qty=30",
        serde_json::to_string(&pfe3).unwrap_or_default()))["portfolio"].clone();
    pfe3 = http_get(port, &format!("/portfolio_sell?pf={}&asset=1&qty=20",
        serde_json::to_string(&pfe3).unwrap_or_default()))["portfolio"].clone();
    let ve3 = http_get(port, &format!("/portfolio_value?pf={}",
        serde_json::to_string(&pfe3).unwrap_or_default()))["value"].as_i64().unwrap_or(0);
    let re3 = http_get(port, &format!("/portfolio_risk?pf={}",
        serde_json::to_string(&pfe3).unwrap_or_default()))["risk"].as_i64().unwrap_or(0);
    let ce3 = pfe3[0].as_i64().unwrap_or(-1);
    let qa_e3 = pfe3[1].as_i64().unwrap_or(-1);
    let qb_e3 = pfe3[2].as_i64().unwrap_or(-1);
    check!("HTTP /etr_chain restored", ce3 == 100 && qa_e3 == 0 && qb_e3 == 0);
    ...
    // 48. 全业务链七链守恒对账 (v0.497) — 发单→验收→提现后：任务数=1、配额
    //     remaining ≥ 0、escrow=0、available ≥ 0、契分/贡献/勋章联动（与
    //     Python --full-business-seven-link-test 对应，INV-SK-21 语义）
    let escrow_before9 = http_get(port, "/stats")["platform_points"][0].as_i64().unwrap_or(0);
    let avail_before9 = http_get(port, "/stats")["platform_points"][1].as_i64().unwrap_or(0);
    let _ = http_get(port, "/post?author=7&bounty=100");
    let tid21 = http_get(port, "/tasks")["tasks"].as_array()
        .and_then(|a| a.last())
        .map(|t| t["task_id"].as_u64().unwrap_or(0)).unwrap_or(0);
    let _ = http_get(port, &format!("/claim?task={tid21}&hunter=3"));
    let _ = http_get(port, &format!("/submit?task={tid21}"));
    let r21 = http_get(port, &format!("/accept?task={tid21}&caller=7"));
    let w21 = http_get(port, "/withdraw?user=3&amount=40");
    let p21 = w21["points"].clone();
    let q21 = http_get(port, "/me?user=7")["quota"][1].as_i64().unwrap_or(-1);
    let b21 = http_get(port, "/badge?user=3")["badge"].as_i64().unwrap_or(0);
    let t21 = http_get(port, "/tasks")["tasks"].as_array().map(|a| a.len()).unwrap_or(0);
    check!("HTTP /fbs2_chain tasks", t21 >= 1);
    check!("HTTP /fbs2_chain quota", q21 >= 0 && q21 <= 50);
    check!("HTTP /fbs2_chain escrow",
           p21[0].as_i64().unwrap_or(-1) == escrow_before9);
    check!("HTTP /fbs2_chain available",
           p21[1].as_i64().unwrap_or(-1) == avail_before9 + 60
           && p21[1].as_i64().unwrap_or(-1) >= 0);
    check!("HTTP /fbs2_chain credit",
           r21["credit"].as_i64().unwrap_or(0) >= 100 + 5);
    check!("HTTP /fbs2_chain contribution", r21["contribution"].as_i64().unwrap_or(0) >= 10);
    check!("HTTP /fbs2_chain badge", b21 >= 1);

    // 49. 双资产等量买卖对消-估值-风险-恢复-对消循环七链对账 (v0.507) —
    //     buy(0,30)→buy(1,20)→sell(0,30)→sell(1,20) 两轮（对消循环）后现金/
    //     资产完全恢复、估值守恒、估值 ≥ 风险且对消可重复（循环估值=初始，与
    //     Python --dual-asset-equal-trade-vr-restore-cycle-test 对应，INV-PF-15
    //     语义）
    let mut pfe4 = http_get(port, "/portfolio_new?cash=100")["portfolio"].clone();
    pfe4 = http_get(port, &format!("/portfolio_buy?pf={}&asset=0&qty=30",
        serde_json::to_string(&pfe4).unwrap_or_default()))["portfolio"].clone();
    pfe4 = http_get(port, &format!("/portfolio_buy?pf={}&asset=1&qty=20",
        serde_json::to_string(&pfe4).unwrap_or_default()))["portfolio"].clone();
    pfe4 = http_get(port, &format!("/portfolio_sell?pf={}&asset=0&qty=30",
        serde_json::to_string(&pfe4).unwrap_or_default()))["portfolio"].clone();
    pfe4 = http_get(port, &format!("/portfolio_sell?pf={}&asset=1&qty=20",
        serde_json::to_string(&pfe4).unwrap_or_default()))["portfolio"].clone();
    // 对消循环：再跑一轮等量买卖
    pfe4 = http_get(port, &format!("/portfolio_buy?pf={}&asset=0&qty=30",
        serde_json::to_string(&pfe4).unwrap_or_default()))["portfolio"].clone();
    ...
    pfe4 = http_get(port, &format!("/portfolio_sell?pf={}&asset=1&qty=20",
        serde_json::to_string(&pfe4).unwrap_or_default()))["portfolio"].clone();
    let ve4 = http_get(port, &format!("/portfolio_value?pf={}",
        serde_json::to_string(&pfe4).unwrap_or_default()))["value"].as_i64().unwrap_or(0);
    let re4 = http_get(port, &format!("/portfolio_risk?pf={}",
        serde_json::to_string(&pfe4).unwrap_or_default()))["risk"].as_i64().unwrap_or(0);
    let ce4 = pfe4[0].as_i64().unwrap_or(-1);
    let qa_e4 = pfe4[1].as_i64().unwrap_or(-1);
    let qb_e4 = pfe4[2].as_i64().unwrap_or(-1);
    check!("HTTP /etrc_chain restored", ce4 == 100 && qa_e4 == 0 && qb_e4 == 0);
    check!("HTTP /etrc_chain value", ve4 == 100);
    ...
    // 51. 全业务链八链守恒对账 (v0.527) — 发单→验收→提现后：任务数=1、兑现
    //     =40（available 减 40）、配额 remaining ≥ 0、escrow=0、available ≥ 0、
    //     契分/贡献/勋章联动（与 Python --full-business-eight-link-test 对应，
    //     INV-SK-22 语义）
    let escrow_before10 = http_get(port, "/stats")["platform_points"][0].as_i64().unwrap_or(0);
    let avail_before10 = http_get(port, "/stats")["platform_points"][1].as_i64().unwrap_or(0);
    let _ = http_get(port, "/post?author=7&bounty=100");
    let tid22 = http_get(port, "/tasks")["tasks"].as_array()
        .and_then(|a| a.last())
        .map(|t| t["task_id"].as_u64().unwrap_or(0)).unwrap_or(0);
    let _ = http_get(port, &format!("/claim?task={tid22}&hunter=3"));
    let _ = http_get(port, &format!("/submit?task={tid22}"));
    let r22 = http_get(port, &format!("/accept?task={tid22}&caller=7"));
    let avail_after_accept = r22["points"][1].as_i64().unwrap_or(0);
    let w22 = http_get(port, "/withdraw?user=3&amount=40");
    let p22 = w22["points"].clone();
    let q22 = http_get(port, "/me?user=7")["quota"][1].as_i64().unwrap_or(-1);
    let b22 = http_get(port, "/badge?user=3")["badge"].as_i64().unwrap_or(0);
    let t22 = http_get(port, "/tasks")["tasks"].as_array().map(|a| a.len()).unwrap_or(0);
    check!("HTTP /fbe_chain tasks", t22 >= 1);
    check!("HTTP /fbe_chain cashed",
           p22[1].as_i64().unwrap_or(-1) == avail_after_accept - 40);
    check!("HTTP /fbe_chain quota", q22 >= 0 && q22 <= 50);
    check!("HTTP /fbe_chain escrow",
           p22[0].as_i64().unwrap_or(-1) == escrow_before10);
    check!("HTTP /fbe_chain available",
           p22[1].as_i64().unwrap_or(-1) == avail_before10 + 60
           && p22[1].as_i64().unwrap_or(-1) >= 0);
    check!("HTTP /fbe_chain credit",
